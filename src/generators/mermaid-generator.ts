...
import type { MermaidOptions, DiagramType, SchemaEntity } from '../types/mermaid-types';
 ...
 */
function parseSchemaToEntities(schema: z.ZodTypeAny, options: Required<MermaidOptions>, parentFieldName?: string): SchemaEntity[] {
 ...
 */
function getEntityName(schema: z.ZodTypeAny, options: Required<MermaidOptions>, parentFieldName?: string): string {
...
function getFieldValidation(schema: z.ZodTypeAny): string[] {
  ...
  if (type === 'optional' || type === 'nullable' || type === 'default' || type === 'lazy') {
    const unwrappedSchema = type === 'optional' 
  ...
  if (type === 'string') {
    ...
    const minLengthCheck = checks.find((check: any) => check.constructor.name === '$ZodCheckMinLength');
    const maxLengthCheck = checks.find((check: any) => check.constructor.name === '$ZodCheckMaxLength');
  ...
  if (type === 'number') {
    ...
    if (checks.some((check: any) => check.constructor.name === '$ZodCheckGreaterThan' && check.value === 0)) {
...
function generateERDiagram(entities: SchemaEntity[], options: Required<MermaidOptions>): string {
  ...
  for (const entity of entities) {
    ...
    for (const field of entity.fields) {
      ...
      const validationString = options.includeValidation && validation?.length
        ? ` "${validation.join(', ')}"`
        : '';
  ...
  for (const entity of entities) {
    for (const field of entity.fields) {
      ...
      if (referencedEntity && field.type !== 'string' && field.type !== 'number' && field.type !== 'boolean' && field.type !== 'date' && !field.type.endsWith('[]')) {
        const relationshipType = field.isOptional ? '||--o{' : '||--||';
        lines.push(`    ${entity.name} ${relationshipType} ${referencedEntity.name} : "${field.name}"`);
      }
 ...
 */
function generateFlowchartDiagram(entities: SchemaEntity[], options: Required<MermaidOptions>): string {
...
import { ZodMermaidError } from '../utils/errors';
